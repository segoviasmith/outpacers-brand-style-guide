...
var applyDecorators = (storyFn, decorators3) => Gr((context) => import_react.default.createElement(storyFn, context), decorators3);
...
var Wrapper = FRAMEWORK_OPTIONS?.strictMode ? import_react2.StrictMode : import_react2.Fragment;
var actQueue = [];
var isActing = false;
...
async function renderToCanvas({ storyContext, unboundStoryFn, showMain, showException, forceRemount }, canvasElement) {
  let { renderElement, unmountElement } = await import("./react-18-3FZZZGM6.js"), Story = unboundStoryFn, content = storyContext.parameters.__isPortableStory ? import_react2.default.createElement(Story, { ...storyContext }) : import_react2.default.createElement(ErrorBoundary, { key: storyContext.id, showMain, showException }, import_react2.default.createElement(Story, { ...storyContext })), element = Wrapper ? import_react2.default.createElement(Wrapper, null, content) : content;
...
var _getWrappedComponentDisplayName = function(Component2) {
  switch (true) {
    case !!Component2.displayName:
      return Component2.displayName;
    case Component2.$$typeof === import_react_is.Memo:
      return _getWrappedComponentDisplayName(Component2.type);
    case Component2.$$typeof === import_react_is.ForwardRef:
      return _getWrappedComponentDisplayName(Component2.render);
    default:
...
var formatComplexDataStructure = function(value, inline, lvl, options) {
  var normalizedValue = sortObject(value), stringifiedValue = (0, import_pretty_print_object.prettyPrint)(normalizedValue, { transform: function(currentObj, prop, originalResult) {
    ...
    return currentValue && (0, import_react3.isValidElement)(currentValue) ? formatTreeNode(_parseReactElement(currentValue, options), true, lvl, options) : typeof currentValue == "function" ? formatFunction(currentValue, options) : originalResult;
  } });
...
var formatPropValue = function(propValue, inline, lvl, options) {
  ...
  return typeof propValue == "function" ? "{".concat(formatFunction(propValue, options), "}") : (0, import_react3.isValidElement)(propValue) ? "{".concat(formatTreeNode(_parseReactElement(propValue, options), true, lvl, options), "}") : propValue instanceof Date ? isNaN(propValue.valueOf()) ? "{new Date(NaN)}" : '{new Date("'.concat(propValue.toISOString(), '")}') : isPlainObject(propValue) || Array.isArray(propValue) ? "{".concat(formatComplexDataStructure(propValue, inline, lvl, options), "}") : "{".concat(String(propValue), "}");
};
...
var jsxDecorator = (storyFn, context) => {
  let jsx = is(void 0), story = storyFn(), skip = skipJsxRender(context), options = { ...defaultOpts, ...context?.parameters.jsx || {} }, storyJsx = context.originalStoryFn(context.args, context);
  return Ot(() => {
    if (skip) return;
    let sourceJsx = mdxToJsx(storyJsx), rendered = renderJsx(sourceJsx, options);
    rendered && jsx.current !== rendered && (Un(rendered, context), jsx.current = rendered);
  }), story;
};
...
function setProjectAnnotations(projectAnnotations) {
  return Ws(INTERNAL_DEFAULT_PROJECT_ANNOTATIONS), Xs(projectAnnotations);
}
var INTERNAL_DEFAULT_PROJECT_ANNOTATIONS = K([entry_preview_exports, entry_preview_argtypes_exports, { renderToCanvas: async (renderContext, canvasElement) => {
